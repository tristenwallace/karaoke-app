import {
  initializeSongQueueHandlers,
  initializeButtonHandlers,
} from './modules/songQueue.js';
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  //sessionCode provided by Jinga in session.html
  initializeButtonHandlers(sessionCode);
  initializeSongQueueHandlers(sessionCode);

  // Close alerts with a fade effect after 3 seconds
  setTimeout(() => {
    const alerts = document.querySelectorAll(
      '.alert:not(#videoContainer .alert)',
    );
