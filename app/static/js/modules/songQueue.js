...
export function initializeButtonHandlers(sessionCode) {
  const nextSongButton = document.getElementById('nextSong');

  nextSongButton.addEventListener('click', function () {
    const currentSongItem = document.querySelector(
      '#songQueue .list-group-item.current',
    );
    if (currentSongItem) {
      ...
      fetch(`/session/${sessionCode}/next_song`, {
        ...
        .then((data) => {
          ...
          if (data.success) {
            ...
            const nextSongItem = document.querySelector(
              `#songQueue .list-group-item[data-id="${data.nextSongId}"]`,
            );
            if (nextSongItem) {
            ...
            messageContainer.innerHTML = `
                    ...
                    `;
            document
              .querySelector('.container-fluid.pt-5')
  ...
  songQueueItems.forEach((item) => {
    ...
    item.addEventListener('drop', () => {
      ...
      if (this !== draggedItem) {
        ...
        const draggedPos = Array.from(songQueueList.children).indexOf(
          draggedItem,
        );
